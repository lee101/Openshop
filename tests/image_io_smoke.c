...
int main(void) {
    ...
    }
    {
        ...
        if (!canvas_load_bmp_with_result(&dst, "test-artifacts/image-io-roundtrip.bmp", 0xFFFFFFFF, &changed)) {
            fprintf(stderr, "canvas_load_bmp_with_result no-op reload should succeed\n");
            ok = 0;
            goto cleanup;
        }
        if (changed) {
            fprintf(stderr, "canvas_load_bmp_with_result no-op reload should report unchanged\n");
            ok = 0;
            goto cleanup;
        }
    }
    if (canvas_load_bmp(&dst, "test-artifacts/image-io-roundtrip.bmp", 0xFFFFFFFF)) {
        fprintf(stderr, "canvas_load_bmp no-op reload should return false\n");
        ok = 0;
        goto cleanup;
    }
    ok = ok && expect_pixel_eq("reloaded_00", canvas_get_pixel(&dst, 0, 0), 0xFF112233);
    ok = ok && expect_pixel_eq("reloaded_extra_col", canvas_get_pixel(&dst, 3, 0), 0xFFFFFFFF);
