...
static int test_layers_basic(void) {
    ...
    if (layer_stack_show(&stack, -1)) {
        fprintf(stderr, "show should fail on invalid source index\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (stack.active_layer != 0 || stack.solo_index != -1 || stack.layers[0].visible != 1 || stack.layers[1].visible != 1) {
        fprintf(stderr, "invalid show should preserve stack bookkeeping\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (layer_stack_toggle_solo(&stack, -1)) {
        fprintf(stderr, "toggle solo should fail on invalid source index\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (stack.active_layer != 0 || stack.solo_index != -1 || stack.layers[0].visible != 1 || stack.layers[1].visible != 1) {
        fprintf(stderr, "invalid toggle solo should preserve stack bookkeeping\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (layer_stack_toggle_lock(&stack, -1)) {
        fprintf(stderr, "toggle lock should fail on invalid source index\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (stack.active_layer != 0 || stack.solo_index != -1 || stack.layers[0].visible != 1 || stack.layers[1].visible != 1 ||
        stack.layers[0].locked != 0 || stack.layers[1].locked != 0) {
        fprintf(stderr, "invalid toggle lock should preserve stack bookkeeping\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    canvas_set_pixel(&stack.layers[MAX_LAYERS - 1].canvas, 0, 0, 0xFFABCDEF);
    stack.solo_index = 1;
    ...
    if (!expect_pixel_eq("full_stack_failure_preserves_top_pixel", canvas_get_pixel(&stack.layers[MAX_LAYERS - 1].canvas, 0, 0), 0xFFABCDEF)) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    layer_stack_composite(&stack, &composite, 0xFFFFFFFF);
    if (!expect_pixel_eq("clear_layer_composite", canvas_get_pixel(&composite, 8, 8), 0xFFABCDEF)) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_set_opacity(&stack, 1, 35)) {
        fprintf(stderr, "set clear-layer opacity failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    layer_stack_composite(&stack, &composite, 0xFFFFFFFF);
    if (!expect_pixel_eq("clear_layer_composite_opacity", canvas_get_pixel(&composite, 8, 8), 0xFFE2EEF9)) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_set_opacity(&stack, 1, 0)) {
        fprintf(stderr, "set clear-layer zero opacity failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    layer_stack_composite(&stack, &composite, 0xFFFFFFFF);
    if (!expect_pixel_eq("clear_layer_composite_zero_opacity", canvas_get_pixel(&composite, 8, 8), clear_neighbor_pixel)) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_set_opacity(&stack, 1, locked_layer_opacity)) {
        fprintf(stderr, "restore clear-layer opacity failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!layer_stack_clear_layer(&stack, 1, 0xFF135724)) {
        fprintf(stderr, "clear hidden layer failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    layer_stack_composite(&stack, &composite, 0xFFFFFFFF);
    if (!expect_pixel_eq("clear_hidden_solo_layer_composite", canvas_get_pixel(&composite, 8, 8), 0xFF135724)) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (layer_stack_add(&stack, "Upper Hidden Solo Merge", 0x00000000) != 1) {
        fprintf(stderr, "add hidden solo merge layer failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!layer_stack_toggle_solo(&stack, 1) || !layer_stack_toggle_visibility(&stack, 1)) {
        fprintf(stderr, "prepare hidden solo merge state failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_merge_down(&stack, 1)) {
        fprintf(stderr, "merge down hidden solo failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (stack.layer_count != 1 || stack.active_layer != 0 || stack.solo_index != 0 ||
        ...
        fprintf(stderr, "merge down hidden solo bookkeeping failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("merge_down_hidden_solo_pixel", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF00FF00)) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_toggle_solo(&stack, 0) || stack.solo_index != -1) {
        fprintf(stderr, "clear solo after hidden merge down failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (layer_stack_add(&stack, "Upper Hidden Merge", 0x00000000) != 1) {
        fprintf(stderr, "add hidden merge layer failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    canvas_clear(&stack.layers[1].canvas, 0xFF00FF00);
    if (!layer_stack_toggle_visibility(&stack, 1) || stack.layers[1].visible) {
        fprintf(stderr, "prepare hidden merge state failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_merge_down(&stack, 1)) {
        fprintf(stderr, "merge down hidden non-solo failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (stack.layer_count != 1 || stack.active_layer != 0 || stack.solo_index != -1 ||
        !stack.layers[0].visible || stack.layers[0].opacity_percent != 100) {
        fprintf(stderr, "merge down hidden non-solo bookkeeping failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("merge_down_hidden_non_solo_pixel", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF00FF00)) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (layer_stack_add(&stack, "Upper Hidden Merge Up", 0x00000000) != 1) {
        fprintf(stderr, "add hidden merge-up layer failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    canvas_clear(&stack.layers[0].canvas, 0xFF123456);
    canvas_clear(&stack.layers[1].canvas, 0xFF00FF00);
    if (!layer_stack_toggle_visibility(&stack, 0) || stack.layers[0].visible) {
        fprintf(stderr, "prepare hidden merge-up state failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    stack.active_layer = 0;
    if (!layer_stack_merge_up(&stack, 0)) {
        fprintf(stderr, "merge up hidden non-solo failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (stack.layer_count != 1 || stack.active_layer != 0 || stack.solo_index != -1 ||
        !stack.layers[0].visible || stack.layers[0].opacity_percent != 100) {
        fprintf(stderr, "merge up hidden non-solo bookkeeping failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("merge_up_hidden_non_solo_pixel", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF00FF00)) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (layer_stack_move(&stack, -1, 1)) {
        fprintf(stderr, "move layer with invalid source index should fail\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (stack.active_layer != 0 || !stack.layers[0].locked || strcmp(stack.layers[0].name, "Background Copy") != 0 ||
        stack.solo_index != 0 || stack.layers[0].visible || stack.layers[0].opacity_percent != 35) {
        fprintf(stderr, "failed invalid-source move should preserve bookkeeping\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (layer_stack_move_to(&stack, 0, 0)) {
        fprintf(stderr, "move layer to same exact index should fail\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (stack.active_layer != 0 || !stack.layers[0].locked || strcmp(stack.layers[0].name, "Background Copy") != 0 ||
        stack.solo_index != 0 || stack.layers[0].visible || stack.layers[0].opacity_percent != 35) {
        fprintf(stderr, "failed exact move-to should preserve bookkeeping\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (layer_stack_move_to(&stack, -1, 0)) {
        fprintf(stderr, "move layer with invalid source index should fail\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    ...
    if (stack.active_layer != 0 || !stack.layers[0].locked || strcmp(stack.layers[0].name, "Background Copy") != 0 ||
        stack.solo_index != 0 || stack.layers[0].visible || stack.layers[0].opacity_percent != 35) {
        fprintf(stderr, "failed invalid-source move-to should preserve bookkeeping\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    int final_layer_visible = stack.layers[0].visible;
    int final_layer_locked = stack.layers[0].locked;
    int final_layer_opacity = stack.layers[0].opacity_percent;
    int final_layer_solo = stack.solo_index;
    ...
    if (stack.layer_count != 1 || stack.active_layer != 0 || stack.solo_index != final_layer_solo ||
        stack.layers[0].visible != final_layer_visible || stack.layers[0].locked != final_layer_locked ||
        stack.layers[0].opacity_percent != final_layer_opacity || strcmp(stack.layers[0].name, final_layer_name) != 0 ||
    ...
    if (layer_stack_add(&stack, "Flatten Hidden Non Solo", 0x00000000) != 1) {
        fprintf(stderr, "add hidden non-solo flatten layer failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    canvas_clear(&stack.layers[1].canvas, 0x8000FF00);
    if (!layer_stack_toggle_visibility(&stack, 1) || stack.layers[1].visible) {
        fprintf(stderr, "prepare hidden non-solo flatten state failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_flatten(&stack, 0xFFFFFFFF)) {
        fprintf(stderr, "flatten hidden non-solo failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (stack.layer_count != 1 || stack.active_layer != 0 || stack.solo_index != -1 ||
        !stack.layers[0].visible || stack.layers[0].locked || stack.layers[0].opacity_percent != 100) {
        fprintf(stderr, "flatten hidden non-solo bookkeeping failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("flatten_hidden_non_solo_pixel", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF123456)) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!layer_stack_toggle_solo(&stack, 1) || !layer_stack_toggle_visibility(&stack, 1)) {
        fprintf(stderr, "prepare hidden solo stamp source failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_stamp_visible_into(&stack, 0, 0xFFFFFFFF)) {
        fprintf(stderr, "stamp visible into hidden solo target failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("stamp_hidden_solo_pixel", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFFBFFFBF) ||
        !stack.layers[0].visible || stack.layers[0].locked || stack.layers[0].opacity_percent != 100 || stack.solo_index != 1) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_toggle_solo(&stack, 1) || stack.solo_index != -1 || !layer_stack_show(&stack, 1)) {
        fprintf(stderr, "restore hidden solo stamp source failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (layer_stack_stamp_visible_new(&stack, "Hidden Solo Stamp", 0xFFFFFFFF) != 2) {
        fprintf(stderr, "stamp visible new hidden solo layer failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("stamp_visible_new_hidden_solo_pixel", canvas_get_pixel(&stack.layers[2].canvas, 0, 0), 0xFF0D6740) ||
        !stack.layers[2].visible || stack.layers[2].locked || stack.layers[2].opacity_percent != 100 || stack.solo_index != 1) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_delete(&stack, 2) || stack.layer_count != 2) {
        fprintf(stderr, "cleanup hidden solo stamp visible new layer failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_toggle_solo(&stack, 1) || stack.solo_index != -1 || !layer_stack_show(&stack, 1)) {
        fprintf(stderr, "restore hidden solo stamp visible new state failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_toggle_visibility(&stack, 1) || stack.layers[1].visible) {
        fprintf(stderr, "prepare hidden non-solo stamp visible new failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (layer_stack_stamp_visible_new(&stack, "Hidden Visible Stamp", 0xFFFFFFFF) != 2) {
        fprintf(stderr, "stamp visible new hidden non-solo layer failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("stamp_visible_new_hidden_non_solo_pixel", canvas_get_pixel(&stack.layers[2].canvas, 0, 0), 0xFF123456) ||
        !stack.layers[2].visible || stack.layers[2].locked || stack.layers[2].opacity_percent != 100 || stack.solo_index != -1) {
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    ...
    if (!layer_stack_delete(&stack, 2) || stack.layer_count != 2) {
        fprintf(stderr, "cleanup hidden non-solo stamp visible new layer failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_stack_show(&stack, 1)) {
        fprintf(stderr, "restore hidden non-solo stamp visible new state failed\n");
        canvas_free(&composite);
        layer_stack_free(&stack);
        return 0;
    }
