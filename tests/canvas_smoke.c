...
static int test_layer_history_record_snapshot_duplicate_keeps_capacity_state(void) {
    LayerStack stack;
    if (!layer_stack_init(&stack, 2, 2, 0xFFFFFFFF)) {
        fprintf(stderr, "history capacity duplicate snapshot init failed\n");
        return 0;
    }
    ...
    for (int i = 0; i < HISTORY_CAPACITY; i++) {
        LayerSnapshot snapshot = {0};
        if (!layer_snapshot_capture(&snapshot, &stack)) {
            fprintf(stderr, "history capacity duplicate snapshot capture failed at %d\n", i);
            layer_history_reset(&history);
            layer_stack_free(&stack);
            return 0;
        }
        if (!layer_history_record_snapshot(&history, &snapshot)) {
            fprintf(stderr, "history capacity duplicate snapshot record failed at %d\n", i);
            layer_history_reset(&history);
            layer_stack_free(&stack);
            return 0;
        }
        canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF500000u | (uint32_t)i);
    }
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF500000u | (uint32_t)(HISTORY_CAPACITY - 2));

    if (!layer_snapshot_capture(&history.redo[history.redo_count++], &stack)) {
        fprintf(stderr, "history capacity duplicate snapshot redo seed failed\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    LayerSnapshot snapshot = {0};
    if (!layer_snapshot_capture(&snapshot, &stack)) {
        fprintf(stderr, "history capacity duplicate snapshot final capture failed\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (layer_history_record_snapshot(&history, &snapshot)) {
        fprintf(stderr, "history capacity duplicate snapshot should not record identical top state\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (!snapshot_is_reset(&snapshot)) {
        fprintf(stderr, "history capacity duplicate snapshot should reset discarded caller snapshot\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (history.undo_count != HISTORY_CAPACITY || history.redo_count != 1) {
        fprintf(stderr, "history capacity duplicate snapshot should preserve undo/redo counts\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!expect_pixel_eq("history_capacity_duplicate_snapshot_oldest_retained", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFFFFFFFF)) {
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
...
static int test_layer_history_undo_redo_preserve_full_destination_on_apply_failure(void) {
    LayerStack stack;
    if (!layer_stack_init(&stack, 2, 2, 0xFFFFFFFF)) {
        fprintf(stderr, "history full-destination rollback init failed\n");
        return 0;
    }
    ...
    if (!seed_history_snapshots_with_pixel_series(&stack, redo_stack, &redo_count, 0xFF110000u, "history full-destination redo seed")) {
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_snapshot_capture(&undo_stack[undo_count++], &stack)) {
        fprintf(stderr, "history full-destination undo capture failed\n");
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (layer_history_undo(&stack, undo_stack, &undo_count, redo_stack, &redo_count)) {
        fprintf(stderr, "history full-destination undo should fail when apply fails\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    ...
    if (undo_count != 1 || redo_count != HISTORY_CAPACITY) {
        fprintf(stderr, "history full-destination undo should restore destination count after rollback\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_full_destination_undo_preserves_oldest_redo", redo_stack[0].pixels[0], 0xFF110000) ||
        !expect_pixel_eq("history_full_destination_undo_preserves_tail_redo", redo_stack[redo_count - 1].pixels[0], 0xFF110013) ||
        !expect_pixel_eq("history_full_destination_undo_keeps_canvas", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF556677)) {
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    ...
    layer_history_clear(redo_stack, &redo_count);
    layer_history_clear(undo_stack, &undo_count);
    if (!seed_history_snapshots_with_pixel_series(&stack, undo_stack, &undo_count, 0xFF220000u, "history full-destination undo seed")) {
        layer_history_clear(undo_stack, &undo_count);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_snapshot_capture(&redo_stack[redo_count++], &stack)) {
        fprintf(stderr, "history full-destination redo capture failed\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_stack_free(&stack);
        return 0;
    }
    redo_stack[0].width++;
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF8899AA);

    if (layer_history_redo(&stack, undo_stack, &undo_count, redo_stack, &redo_count)) {
        fprintf(stderr, "history full-destination redo should fail when apply fails\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    ...
    if (undo_count != HISTORY_CAPACITY || redo_count != 1) {
        fprintf(stderr, "history full-destination redo should restore destination count after rollback\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_full_destination_redo_preserves_oldest_undo", undo_stack[0].pixels[0], 0xFF220000) ||
        !expect_pixel_eq("history_full_destination_redo_preserves_tail_undo", undo_stack[undo_count - 1].pixels[0], 0xFF220013) ||
        !expect_pixel_eq("history_full_destination_redo_keeps_canvas", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF8899AA)) {
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
...
static int test_layer_history_step_undo_redo_preserve_full_destination_on_apply_failure(void) {
    LayerStack stack;
    if (!layer_stack_init(&stack, 2, 2, 0xFFFFFFFF)) {
        fprintf(stderr, "history step full-destination rollback init failed\n");
        return 0;
    }

    LayerHistory history = {0};
    if (!seed_history_snapshots_with_pixel_series(&stack, history.redo, &history.redo_count, 0xFF330000u, "history step full-destination redo seed")) {
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_snapshot_capture(&history.undo[history.undo_count++], &stack)) {
        fprintf(stderr, "history step full-destination undo capture failed\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    history.undo[0].width++;
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF123456);

    if (layer_history_step_undo(&history, &stack)) {
        fprintf(stderr, "history step full-destination undo should fail when apply fails\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (history.undo_count != 1 || history.redo_count != HISTORY_CAPACITY) {
        fprintf(stderr, "history step full-destination undo should restore destination count after rollback\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_step_full_destination_undo_preserves_oldest_redo", history.redo[0].pixels[0], 0xFF330000) ||
        !expect_pixel_eq("history_step_full_destination_undo_preserves_tail_redo", history.redo[history.redo_count - 1].pixels[0], 0xFF330013) ||
        !expect_pixel_eq("history_step_full_destination_undo_keeps_canvas", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF123456)) {
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!seed_history_snapshots_with_pixel_series(&stack, history.undo, &history.undo_count, 0xFF440000u, "history step full-destination undo seed")) {
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (!layer_snapshot_capture(&history.redo[history.redo_count++], &stack)) {
        fprintf(stderr, "history step full-destination redo capture failed\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    history.redo[0].width++;
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF654321);

    if (layer_history_step_redo(&history, &stack)) {
        fprintf(stderr, "history step full-destination redo should fail when apply fails\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (history.undo_count != HISTORY_CAPACITY || history.redo_count != 1) {
        fprintf(stderr, "history step full-destination redo should restore destination count after rollback\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_step_full_destination_redo_preserves_oldest_undo", history.undo[0].pixels[0], 0xFF440000) ||
        !expect_pixel_eq("history_step_full_destination_redo_preserves_tail_undo", history.undo[history.undo_count - 1].pixels[0], 0xFF440013) ||
        !expect_pixel_eq("history_step_full_destination_redo_keeps_canvas", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF654321)) {
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
...
static int test_layer_history_low_level_undo_redo_preserve_state_on_capture_failure(void) {
#ifndef OPENSHOP_TESTING
    fprintf(stderr, "history capture failure test requires OPENSHOP_TESTING\n");
    return 0;
#else
    LayerStack stack;
    if (!layer_stack_init(&stack, 4, 4, 0xFFFFFFFF)) {
        fprintf(stderr, "history low-level capture-failure init failed\n");
        return 0;
    }
    ...
    if (!layer_snapshot_capture(&undo_stack[undo_count++], &stack)) {
        fprintf(stderr, "history low-level capture-failure undo seed failed\n");
        layer_stack_free(&stack);
        return 0;
    }
    ...
    layer_snapshot_set_alloc_for_tests(always_fail_snapshot_alloc);
    if (layer_history_undo(&stack, undo_stack, &undo_count, redo_stack, &redo_count)) {
        fprintf(stderr, "history low-level undo should fail when current-state capture fails\n");
        layer_snapshot_set_alloc_for_tests(NULL);
        layer_history_clear(undo_stack, &undo_count);
        layer_stack_free(&stack);
        return 0;
    ...
    layer_snapshot_set_alloc_for_tests(NULL);
    if (undo_count != 1 || redo_count != 0) {
        fprintf(stderr, "history low-level undo should keep counts unchanged after current-state capture failure\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!layer_snapshot_capture(&redo_stack[redo_count++], &stack)) {
        fprintf(stderr, "history low-level capture-failure redo seed failed\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    layer_snapshot_set_alloc_for_tests(always_fail_snapshot_alloc);
    if (layer_history_redo(&stack, undo_stack, &undo_count, redo_stack, &redo_count)) {
        fprintf(stderr, "history low-level redo should fail when current-state capture fails\n");
        layer_snapshot_set_alloc_for_tests(NULL);
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
    ...
    if (!expect_pixel_eq("history_low_level_failed_capture_redo_keeps_canvas", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF010203)) {
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
...
static int test_layer_history_step_undo_redo_preserve_state_on_capture_failure(void) {
    ...
    LayerStack stack;
    if (!layer_stack_init(&stack, 4, 4, 0xFFFFFFFF)) {
        fprintf(stderr, "history step capture-failure init failed\n");
        return 0;
    }
    ...
    layer_snapshot_set_alloc_for_tests(always_fail_snapshot_alloc);
    if (layer_history_step_undo(&history, &stack)) {
        fprintf(stderr, "history step undo should fail when current-state capture fails\n");
        layer_snapshot_set_alloc_for_tests(NULL);
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    ...
    layer_snapshot_set_alloc_for_tests(NULL);
    if (history.undo_count != 2 || history.redo_count != 0) {
        fprintf(stderr, "history step undo should keep counts unchanged after current-state capture failure\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!layer_snapshot_capture(&history.redo[history.redo_count++], &stack)) {
        fprintf(stderr, "history step capture-failure redo seed failed\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    layer_snapshot_set_alloc_for_tests(always_fail_snapshot_alloc);
    if (layer_history_step_redo(&history, &stack)) {
        fprintf(stderr, "history step redo should fail when current-state capture fails\n");
        layer_snapshot_set_alloc_for_tests(NULL);
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
...
static int test_layer_history_low_level_undo_redo_evict_destination_at_capacity(void) {
    LayerStack stack;
    if (!layer_stack_init(&stack, 2, 2, 0xFFFFFFFF)) {
        fprintf(stderr, "history low-level capacity init failed\n");
        return 0;
    }
    ...
    if (!layer_snapshot_capture(&undo_stack[undo_count++], &stack)) {
        fprintf(stderr, "history low-level undo capacity seed failed\n");
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!layer_history_undo(&stack, undo_stack, &undo_count, redo_stack, &redo_count)) {
        fprintf(stderr, "history low-level undo should succeed with full redo stack\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    }
    if (undo_count != 0 || redo_count != HISTORY_CAPACITY) {
        fprintf(stderr, "history low-level undo should keep redo capped at capacity\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_low_level_undo_capacity_applies_snapshot", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFFFFFFFF) ||
        !expect_pixel_eq("history_low_level_undo_capacity_evicts_oldest_redo", redo_stack[0].pixels[0], 0xFF100001) ||
        !expect_pixel_eq("history_low_level_undo_capacity_pushes_current", redo_stack[redo_count - 1].pixels[0], 0xFF010203)) {
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    ...
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF112233);
    if (!layer_snapshot_capture(&redo_stack[redo_count++], &stack)) {
        fprintf(stderr, "history low-level redo capacity seed failed\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!layer_history_redo(&stack, undo_stack, &undo_count, redo_stack, &redo_count)) {
        fprintf(stderr, "history low-level redo should succeed with full undo stack\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    ...
    if (undo_count != HISTORY_CAPACITY || redo_count != 0) {
        fprintf(stderr, "history low-level redo should keep undo capped at capacity\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_low_level_redo_capacity_applies_snapshot", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF112233) ||
        !expect_pixel_eq("history_low_level_redo_capacity_evicts_oldest_undo", undo_stack[0].pixels[0], 0xFF200001) ||
        !expect_pixel_eq("history_low_level_redo_capacity_pushes_current", undo_stack[undo_count - 1].pixels[0], 0xFF040506)) {
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
...
static int test_layer_history_low_level_undo_redo_duplicate_keeps_capacity_state(void) {
    LayerStack stack;
    if (!layer_stack_init(&stack, 2, 2, 0xFFFFFFFF)) {
        fprintf(stderr, "history low-level duplicate capacity init failed\n");
        return 0;
    }
    ...
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF556677);
    if (!layer_snapshot_capture(&undo_stack[undo_count++], &stack)) {
        fprintf(stderr, "history low-level duplicate undo seed failed\n");
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    }
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF900013);

    if (!layer_history_undo(&stack, undo_stack, &undo_count, redo_stack, &redo_count)) {
        fprintf(stderr, "history low-level duplicate undo should succeed\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    ...
    if (undo_count != 0 || redo_count != HISTORY_CAPACITY) {
        fprintf(stderr, "history low-level duplicate undo should preserve destination count at capacity\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_low_level_duplicate_undo_applies_snapshot", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF556677) ||
        !expect_pixel_eq("history_low_level_duplicate_undo_oldest_retained", redo_stack[0].pixels[0], 0xFF900000) ||
        !expect_pixel_eq("history_low_level_duplicate_undo_tail_unchanged", redo_stack[redo_count - 1].pixels[0], 0xFF900013)) {
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    ...
    if (!layer_snapshot_capture(&redo_stack[redo_count++], &stack)) {
        fprintf(stderr, "history low-level duplicate redo snapshot seed failed\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_stack_free(&stack);
        return 0;
    }
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFFA00013);

    if (!layer_history_redo(&stack, undo_stack, &undo_count, redo_stack, &redo_count)) {
        fprintf(stderr, "history low-level duplicate redo should succeed\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    ...
    if (undo_count != HISTORY_CAPACITY || redo_count != 0) {
        fprintf(stderr, "history low-level duplicate redo should preserve destination count at capacity\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_low_level_duplicate_redo_applies_snapshot", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF99AABB) ||
        !expect_pixel_eq("history_low_level_duplicate_redo_oldest_retained", undo_stack[0].pixels[0], 0xFFA00000) ||
        !expect_pixel_eq("history_low_level_duplicate_redo_tail_unchanged", undo_stack[undo_count - 1].pixels[0], 0xFFA00013)) {
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
...
static int test_layer_history_step_undo_redo_evict_destination_at_capacity(void) {
    LayerStack stack;
    if (!layer_stack_init(&stack, 2, 2, 0xFFFFFFFF)) {
        fprintf(stderr, "history step capacity init failed\n");
        return 0;
    }
    ...
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFFFFFFFF);
    if (!layer_snapshot_capture(&history.undo[history.undo_count++], &stack)) {
        fprintf(stderr, "history step undo capacity seed failed\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!layer_history_step_undo(&history, &stack)) {
        fprintf(stderr, "history step undo should succeed with full redo stack\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (history.undo_count != 0 || history.redo_count != HISTORY_CAPACITY) {
        fprintf(stderr, "history step undo should keep redo capped at capacity\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_step_undo_capacity_applies_snapshot", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFFFFFFFF) ||
        !expect_pixel_eq("history_step_undo_capacity_evicts_oldest_redo", history.redo[0].pixels[0], 0xFF300001) ||
        !expect_pixel_eq("history_step_undo_capacity_pushes_current", history.redo[history.redo_count - 1].pixels[0], 0xFF010203)) {
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF112233);
    if (!layer_snapshot_capture(&history.redo[history.redo_count++], &stack)) {
        fprintf(stderr, "history step redo capacity seed failed\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!layer_history_step_redo(&history, &stack)) {
        fprintf(stderr, "history step redo should succeed with full undo stack\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (history.undo_count != HISTORY_CAPACITY || history.redo_count != 0) {
        fprintf(stderr, "history step redo should keep undo capped at capacity\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_step_redo_capacity_applies_snapshot", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF112233) ||
        !expect_pixel_eq("history_step_redo_capacity_evicts_oldest_undo", history.undo[0].pixels[0], 0xFF400001) ||
        !expect_pixel_eq("history_step_redo_capacity_pushes_current", history.undo[history.undo_count - 1].pixels[0], 0xFF040506)) {
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
...
static int test_layer_history_step_undo_redo_duplicate_keeps_capacity_state(void) {
    LayerStack stack;
    if (!layer_stack_init(&stack, 2, 2, 0xFFFFFFFF)) {
        fprintf(stderr, "history step duplicate capacity init failed\n");
        return 0;
    }
    ...
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF556677);
    if (!layer_snapshot_capture(&history.undo[history.undo_count++], &stack)) {
        fprintf(stderr, "history step duplicate undo seed failed\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF700013);

    if (!layer_history_step_undo(&history, &stack)) {
        fprintf(stderr, "history step duplicate undo should succeed\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (history.undo_count != 0 || history.redo_count != HISTORY_CAPACITY) {
        fprintf(stderr, "history step duplicate undo should preserve destination count at capacity\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_step_duplicate_undo_applies_snapshot", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF556677) ||
        !expect_pixel_eq("history_step_duplicate_undo_oldest_retained", history.redo[0].pixels[0], 0xFF700000) ||
        !expect_pixel_eq("history_step_duplicate_undo_tail_unchanged", history.redo[history.redo_count - 1].pixels[0], 0xFF700013)) {
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF99AABB);
    if (!layer_snapshot_capture(&history.redo[history.redo_count++], &stack)) {
        fprintf(stderr, "history step duplicate redo snapshot seed failed\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF800013);

    if (!layer_history_step_redo(&history, &stack)) {
        fprintf(stderr, "history step duplicate redo should succeed\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (history.undo_count != HISTORY_CAPACITY || history.redo_count != 0) {
        fprintf(stderr, "history step duplicate redo should preserve destination count at capacity\n");
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
    if (!expect_pixel_eq("history_step_duplicate_redo_applies_snapshot", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF99AABB) ||
        !expect_pixel_eq("history_step_duplicate_redo_oldest_retained", history.undo[0].pixels[0], 0xFF800000) ||
        !expect_pixel_eq("history_step_duplicate_redo_tail_unchanged", history.undo[history.undo_count - 1].pixels[0], 0xFF800013)) {
        layer_history_reset(&history);
        layer_stack_free(&stack);
        return 0;
    }
...
static int test_layer_history_push_evicts_oldest_at_capacity(void) {
    LayerStack stack;
    if (!layer_stack_init(&stack, 2, 2, 0xFFFFFFFF)) {
        fprintf(stderr, "history push capacity init failed\n");
        return 0;
    }
    ...
    if (undo_count != HISTORY_CAPACITY || redo_count != 0) {
        fprintf(stderr, "history push capacity should fill undo without redo\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!layer_snapshot_capture(&redo_stack[redo_count++], &stack)) {
        fprintf(stderr, "history push capacity redo seed failed\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (undo_count != HISTORY_CAPACITY || redo_count != 0) {
        fprintf(stderr, "history push capacity should keep undo capped and clear redo\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!expect_pixel_eq("history_push_capacity_oldest_retained", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFF000000)) {
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
    ...
    if (layer_history_undo(&stack, undo_stack, &undo_count, redo_stack, &redo_count)) {
        fprintf(stderr, "history push capacity should evict the original oldest state\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
...
static int test_layer_history_push_duplicate_keeps_capacity_state(void) {
    LayerStack stack;
    if (!layer_stack_init(&stack, 2, 2, 0xFFFFFFFF)) {
        fprintf(stderr, "history push duplicate capacity init failed\n");
        return 0;
    }
    ...
    for (int i = 0; i < HISTORY_CAPACITY; i++) {
        layer_history_push(&stack, undo_stack, &undo_count, redo_stack, &redo_count);
        canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF600000u | (uint32_t)i);
    }
    canvas_set_pixel(&stack.layers[0].canvas, 0, 0, 0xFF600000u | (uint32_t)(HISTORY_CAPACITY - 2));

    if (!layer_snapshot_capture(&redo_stack[redo_count++], &stack)) {
        fprintf(stderr, "history push duplicate capacity redo seed failed\n");
        layer_history_clear(undo_stack, &undo_count);
        layer_stack_free(&stack);
        return 0;
    }
    ...
    if (!expect_pixel_eq("history_push_duplicate_capacity_oldest_retained", canvas_get_pixel(&stack.layers[0].canvas, 0, 0), 0xFFFFFFFF)) {
        layer_history_clear(undo_stack, &undo_count);
        layer_history_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
...
int main(void) {
    ...
    if (!test_layer_history_record_snapshot_duplicate_keeps_capacity_state()) {
        return 1;
    }
    ...
    if (!test_layer_history_undo_redo_preserve_full_destination_on_apply_failure()) {
        return 1;
    }
    if (!test_layer_history_step_undo_redo_preserve_full_destination_on_apply_failure()) {
        return 1;
    }
    ...
    if (!test_layer_history_low_level_undo_redo_duplicate_keeps_capacity_state()) {
        return 1;
    }
    ...
    if (!test_layer_history_step_undo_redo_duplicate_keeps_capacity_state()) {
        return 1;
    }
