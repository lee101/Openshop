...
static int fail_layer_add_after = -1;
static int layer_add_calls = 0;
...
static int stub_layer_add(LayerStack *stack, const char *name, uint32_t clear_color) {
    layer_add_calls++;
    if (fail_layer_add_after >= 0 && layer_add_calls > fail_layer_add_after) {
        return -1;
    }
    return layer_stack_add(stack, name, clear_color);
}

static void install_layer_add_stub(int fail_after) {
    fail_layer_add_after = fail_after;
    layer_add_calls = 0;
    app_history_set_layer_add(fail_after >= 0 ? stub_layer_add : NULL);
}
...
static int test_snapshot_apply_rolls_back_partial_layer_growth(void) {
    LayerStack source;
    LayerStack target;
    Snapshot snapshot = {0};
    ...
    if (!snapshot_from_layers(&snapshot, &source)) {
        fprintf(stderr, "snapshot_from_layers failed\n");
        layer_stack_free(&source);
        layer_stack_free(&target);
        return 0;
    ...
    if (snapshot_apply(&snapshot, &target)) {
        fprintf(stderr, "snapshot_apply should fail when later layer growth fails\n");
        install_layer_add_stub(-1);
        snapshot_free(&snapshot);
        layer_stack_free(&source);
        layer_stack_free(&target);
    ...
    if (!expect_int_eq("apply_layer_add_fail_count", target.layer_count, 1) ||
        !expect_int_eq("apply_layer_add_fail_active", target.active_layer, 0) ||
        !expect_int_eq("apply_layer_add_fail_solo", target.solo_index, -1) ||
        !expect_int_eq("apply_layer_add_fail_visible", target.layers[0].visible, 1) ||
        !expect_int_eq("apply_layer_add_fail_locked", target.layers[0].locked, 0) ||
        !expect_int_eq("apply_layer_add_fail_opacity", target.layers[0].opacity_percent, 100) ||
        !expect_pixel_eq("apply_layer_add_fail_pixel", canvas_get_pixel(&target.layers[0].canvas, 1, 1), 0xFFAABBCC) ||
...
static int test_snapshot_restore_preserves_state_on_late_canvas_allocation_failure(void) {
    LayerStack stack;
    Snapshot undo_stack[MAX_HISTORY];
    Snapshot redo_stack[MAX_HISTORY];
    int undo_count = 0;
    int redo_count = 0;
    uint32_t undo_saved_top_pixel = 0;
    uint32_t redo_oldest_pixel = 0;
    uint32_t redo_newest_pixel = 0;
    ...
    canvas_set_pixel(&stack.layers[2].canvas, 1, 1, 0x80708090);
    snapshot_push(&stack, undo_stack, &undo_count, redo_stack, &redo_count);
    undo_saved_top_pixel = undo_stack[0].pixels[2 * 36 + 7];
    ...
    for (int i = 0; i < MAX_HISTORY; i++) {
        ...
        snapshot.visibility[0] = 1;
        snapshot.locked[0] = 0;
        snapshot.opacity_percent[0] = 100;
        strcpy(snapshot.names[0], "Redo");
        snapshot.pixels = malloc(sizeof(uint32_t));
        if (!snapshot.pixels) {
        ...
        snapshot.pixels[0] = 0xFF000100u + (uint32_t)i;
        redo_stack[i] = snapshot;
    }
    redo_count = MAX_HISTORY;
    redo_oldest_pixel = redo_stack[0].pixels[0];
    redo_newest_pixel = redo_stack[MAX_HISTORY - 1].pixels[0];

    install_canvas_init_stub(1);
    if (snapshot_restore(&stack, undo_stack, &undo_count, redo_stack, &redo_count)) {
        fprintf(stderr, "snapshot_restore should fail when a later canvas allocation fails\n");
        install_canvas_init_stub(-1);
        snapshot_stack_clear(undo_stack, &undo_count);
        snapshot_stack_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
    ...
    if (!expect_int_eq("restore_late_alloc_fail_undo_count", undo_count, 1) ||
        !expect_int_eq("restore_late_alloc_fail_redo_count", redo_count, MAX_HISTORY) ||
        !expect_int_eq("restore_late_alloc_fail_active", stack.active_layer, 0) ||
        !expect_int_eq("restore_late_alloc_fail_solo", stack.solo_index, -1) ||
        !expect_int_eq("restore_late_alloc_fail_visible", stack.layers[0].visible, 1) ||
        !expect_int_eq("restore_late_alloc_fail_locked", stack.layers[1].locked, 0) ||
        !expect_int_eq("restore_late_alloc_fail_opacity", stack.layers[2].opacity_percent, 100) ||
        !expect_pixel_eq("restore_late_alloc_fail_base_pixel", canvas_get_pixel(&stack.layers[0].canvas, 1, 1), 0xFFAABBCC) ||
        !stack.layers[1].canvas.pixels ||
        !expect_pixel_eq("restore_late_alloc_fail_mid_pixel", canvas_get_pixel(&stack.layers[1].canvas, 1, 1), 0x00000000) ||
        !expect_int_eq("restore_late_alloc_fail_top_canvas_width", stack.layers[2].canvas.width, 0) ||
        !expect_int_eq("restore_late_alloc_fail_top_canvas_height", stack.layers[2].canvas.height, 0) ||
        stack.layers[2].canvas.pixels != NULL ||
        !expect_pixel_eq("restore_late_alloc_fail_undo_preserved", undo_stack[0].pixels[2 * 36 + 7], undo_saved_top_pixel) ||
        !expect_pixel_eq("restore_late_alloc_fail_redo_oldest", redo_stack[0].pixels[0], redo_oldest_pixel) ||
        !expect_pixel_eq("restore_late_alloc_fail_redo_newest", redo_stack[MAX_HISTORY - 1].pixels[0], redo_newest_pixel)) {
        snapshot_stack_clear(undo_stack, &undo_count);
        snapshot_stack_clear(redo_stack, &redo_count);
        layer_stack_free(&stack);
        return 0;
...
static int test_snapshot_restore_preserves_full_destination_on_layer_growth_failure(void) {
    LayerStack source;
    LayerStack target;
    Snapshot undo_stack[MAX_HISTORY];
    Snapshot redo_stack[MAX_HISTORY];
    ...
    for (int i = 0; i < MAX_HISTORY; i++) {
        ...
        snapshot.pixels[0] = 0xFF000200u + (uint32_t)i;
        redo_stack[i] = snapshot;
    }
    redo_count = MAX_HISTORY;
    redo_oldest_pixel = redo_stack[0].pixels[0];
    ...
    if (snapshot_restore(&target, undo_stack, &undo_count, redo_stack, &redo_count)) {
        fprintf(stderr, "snapshot_restore should fail when later layer growth fails\n");
        install_layer_add_stub(-1);
        snapshot_stack_clear(undo_stack, &undo_count);
        snapshot_stack_clear(redo_stack, &redo_count);
        layer_stack_free(&source);
    ...
    if (!expect_int_eq("restore_layer_add_fail_undo_count", undo_count, 1) ||
        !expect_int_eq("restore_layer_add_fail_redo_count", redo_count, MAX_HISTORY) ||
        !expect_int_eq("restore_layer_add_fail_layer_count", target.layer_count, 1) ||
        !expect_int_eq("restore_layer_add_fail_active", target.active_layer, 0) ||
        !expect_int_eq("restore_layer_add_fail_solo", target.solo_index, -1) ||
        !expect_int_eq("restore_layer_add_fail_visible", target.layers[0].visible, 1) ||
        !expect_int_eq("restore_layer_add_fail_locked", target.layers[0].locked, 0) ||
        !expect_int_eq("restore_layer_add_fail_opacity", target.layers[0].opacity_percent, 100) ||
        !expect_pixel_eq("restore_layer_add_fail_pixel", canvas_get_pixel(&target.layers[0].canvas, 1, 1), 0xFFAABBCC) ||
        target.layers[1].canvas.pixels != NULL ||
        target.layers[1].name[0] != '\0' ||
        target.layers[2].canvas.pixels != NULL ||
        target.layers[2].name[0] != '\0' ||
        !expect_pixel_eq("restore_layer_add_fail_undo_preserved", undo_stack[0].pixels[2 * 36 + 7], undo_top_pixel) ||
        !expect_pixel_eq("restore_layer_add_fail_redo_oldest", redo_stack[0].pixels[0], redo_oldest_pixel) ||
        !expect_pixel_eq("restore_layer_add_fail_redo_newest", redo_stack[MAX_HISTORY - 1].pixels[0], redo_newest_pixel)) {
        snapshot_stack_clear(undo_stack, &undo_count);
        snapshot_stack_clear(redo_stack, &redo_count);
        layer_stack_free(&source);
        layer_stack_free(&target);
...
int main(void) {
    ...
    if (!test_snapshot_apply_rolls_back_partial_layer_growth()) {
        return 1;
    }
    ...
    if (!test_snapshot_restore_preserves_state_on_late_canvas_allocation_failure()) {
        return 1;
    }
