...
- Failed edits, unchanged edits, failed internal snapshot capture/compare steps, and failed undo/redo step applications all preserve the existing undo/redo stacks and their retained snapshots, including when a full destination stack would have to evict an entry. In those cases the evicted destination entry is restored and any discarded temporary step snapshot is rolled back to an empty state.
