...
/* Adjust contrast of all pixels.
 * step is in the range [-100, +100].  The effective scale factor is
 * (100 + step) / 100, applied around the mid-point 128:
 *   new_channel = clamp((channel - 128) * (100 + step) / 100 + 128, 0, 255)
 * Positive steps increase contrast; negative steps flatten it. Alpha unchanged. */
void canvas_adjust_contrast(Canvas *c, int step) {
    if (!c || !c->pixels || c->width <= 0 || c->height <= 0 || step == 0) {
        return;
    }
    if (step < -100) step = -100;
    if (step > 100) step = 100;

    int factor_num = 100 + step; /* numerator; denominator = 100 */
    size_t count = (size_t)c->width * (size_t)c->height;
    for (size_t i = 0; i < count; i++) {
        uint32_t p = c->pixels[i];
        uint8_t a = (uint8_t)((p >> 24) & 0xFF);
        int r = ((int)((p >> 16) & 0xFF) - 128) * factor_num / 100 + 128;
        int g = ((int)((p >> 8) & 0xFF) - 128) * factor_num / 100 + 128;
        int b = ((int)(p & 0xFF) - 128) * factor_num / 100 + 128;
