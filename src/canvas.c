...
void canvas_grayscale(Canvas *c) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        /* BT.601 luma coefficients (integer approximation) */
        uint8_t luma = (uint8_t)((r * 299 + g * 587 + b * 114 + 500) / 1000);
...
void canvas_posterize(Canvas *c, int levels) {
    if (!c || !c->pixels || c->width <= 0 || c->height <= 0 || levels < 2 || levels > 255) {
        return;
    }
    size_t count = (size_t)c->width * (size_t)c->height;
    for (size_t i = 0; i < count; i++) {
        ...
        uint8_t g = (uint8_t)((p >> 8) & 0xFF);
        uint8_t b = (uint8_t)(p & 0xFF);
        /* Map each channel to the nearest of `levels` evenly-spaced values.
         * Uses rounding (+127) so values are bucketed symmetrically. */
        r = (uint8_t)(((r * (levels - 1) + 127) / 255) * 255 / (levels - 1));
        g = (uint8_t)(((g * (levels - 1) + 127) / 255) * 255 / (levels - 1));
        b = (uint8_t)(((b * (levels - 1) + 127) / 255) * 255 / (levels - 1));
