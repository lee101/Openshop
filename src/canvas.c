...
void canvas_brighten(Canvas *c) { canvas_adjust_brightness(c, 10); }
void canvas_darken(Canvas *c)   { canvas_adjust_brightness(c, -10); }
...
void canvas_adjust_contrast(Canvas *c, int factor_x100) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        int g = (int)((p >> 8) & 0xFF);
        int b = (int)(p & 0xFF);
        r = 128 + (r - 128) * factor_x100 / 100;
        g = 128 + (g - 128) * factor_x100 / 100;
        b = 128 + (b - 128) * factor_x100 / 100;
        if (r < 0) r = 0; else if (r > 255) r = 255;
        if (g < 0) g = 0; else if (g > 255) g = 255;
        if (b < 0) b = 0; else if (b > 255) b = 255;
        c->pixels[i] = ((uint32_t)a << 24) | ((uint32_t)r << 16) | ((uint32_t)g << 8) | (uint32_t)b;
