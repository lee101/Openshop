...
static void update_window_title(SDL_Window *window, const LayerStack *layers, Tool tool, BrushShape brush_shape, int radius, uint32_t color, int opacity_percent) {
    ...
    snprintf(
        ...
        radius,
        opacity_percent,
        g_filter_settings.fill_tolerance,
        g_filter_settings.pixelate_block_size,
...
static void refresh_window_title(
    SDL_Window *window,
    const LayerStack *layers,
    Tool tool,
    BrushShape brush_shape,
...
static int clamp_saturation_delta(int delta) {
    if (delta < SATURATION_MIN) {
        return SATURATION_MIN;
    }
    if (delta > SATURATION_MAX) {
...
static int apply_active_layer_opacity(
    SDL_Window *window,
    LayerStack *layers,
    Snapshot *undo_stack,
    int *undo_count,
    ...
    push_snapshot(layers, undo_stack, undo_count, redo_stack, redo_count);
    if (!layer_stack_set_opacity(layers, layers->active_layer, opacity_percent)) {
        refresh_window_title(window, layers, tool, brush_shape, brush_radius, brush_color, brush_opacity);
        return 0;
    }
    refresh_window_title(window, layers, tool, brush_shape, brush_radius, brush_color, brush_opacity);
        ...
        while (SDL_PollEvent(&e)) {
            ...
            case SDL_KEYDOWN: {
                ...
                if (ctrl && (key == SDLK_MINUS || key == SDLK_KP_MINUS)) {
                    Layer *active = layer_stack_active(&layers);
                    if (active && apply_active_layer_opacity(
                            window, &layers, undo_stack, &undo_count, redo_stack, &redo_count,
                ...
                if (ctrl && key == SDLK_g) {
                    if (apply_canvas_transform(&layers, undo_stack, &undo_count, redo_stack, &redo_count, canvas_edge_detect)) {
                        needs_composite = 1;
                    }
                    break;
                }
                ...
                if (ctrl && shift && key == SDLK_p) {
                    if (apply_canvas_transform_int(&layers, undo_stack, &undo_count, redo_stack, &redo_count,
                            canvas_pixelate, settings.pixelate_block_size)) {
                        needs_composite = 1;
                    }
                    break;
                }
                ...
                if (ctrl && key == SDLK_0) {
                    Layer *active = layer_stack_active(&layers);
                    if (active && active->opacity_percent != 100 && apply_active_layer_opacity(
                            window, &layers, undo_stack, &undo_count, redo_stack, &redo_count, 100,
                            tool, brush_shape, brush_radius, brush_color, brush_opacity)) {
                        needs_composite = 1;
                    }
                ...
                if (ctrl && key == SDLK_f) {
                    ...
                    if (mx >= 0 && my >= 0 && mx < CANVAS_WIDTH && my < CANVAS_HEIGHT) {
                        ...
                        if (!active || active->locked ||
                            !canvas_flood_fill_tol(&active->canvas, mx, my, brush_color, settings.fill_tolerance)) {
                ...
                if (ctrl && key == SDLK_b) {
                    if (apply_canvas_transform_int(&layers, undo_stack, &undo_count, redo_stack, &redo_count,
                            canvas_blur, settings.blur_radius)) {
                        needs_composite = 1;
                    }
                    break;
                }
                ...
                if (ctrl && shift && (key == SDLK_UP || key == SDLK_DOWN)) {
                    if (apply_canvas_transform_int(&layers, undo_stack, &undo_count, redo_stack, &redo_count,
                            ...
                            (key == SDLK_UP) ? settings.filter_adjust_step : -settings.filter_adjust_step)) {
                        needs_composite = 1;
                    }
                    break;
                }
                ...
                if (ctrl && shift && (key == SDLK_LEFT || key == SDLK_RIGHT)) {
                    if (apply_canvas_transform_int(&layers, undo_stack, &undo_count, redo_stack, &redo_count,
                            ...
                            (key == SDLK_RIGHT) ? settings.filter_adjust_step : -settings.filter_adjust_step)) {
                        needs_composite = 1;
                    }
                    break;
                }
