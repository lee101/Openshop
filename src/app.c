        ...
        while (SDL_PollEvent(&e)) {
            ...
            case SDL_KEYDOWN: {
                ...
                if (key == SDLK_b) {
                    ...
                    if (apply_canvas_transform(&layers, undo_stack, &undo_count, redo_stack, &redo_count, canvas_posterize4)) {
                        needs_composite = 1;
                    }
                } else if (key == SDLK_y) {
                    if (apply_canvas_transform(&layers, undo_stack, &undo_count, redo_stack, &redo_count, canvas_threshold_mid)) {
                        needs_composite = 1;
                    }
                } else if (key == SDLK_w) {
                    if (apply_canvas_transform(&layers, undo_stack, &undo_count, redo_stack, &redo_count, canvas_brighten)) {
                        needs_composite = 1;
                    }
                } else if (key == SDLK_s) {
                    if (apply_canvas_transform(&layers, undo_stack, &undo_count, redo_stack, &redo_count, canvas_darken)) {
                        needs_composite = 1;
                    }
                } else if (key == SDLK_a) {
                    if (apply_canvas_transform(&layers, undo_stack, &undo_count, redo_stack, &redo_count, canvas_increase_contrast)) {
                        needs_composite = 1;
                    }
                } else if (key == SDLK_z) {
                    if (apply_canvas_transform(&layers, undo_stack, &undo_count, redo_stack, &redo_count, canvas_decrease_contrast)) {
                        needs_composite = 1;
                    }
                } else if (key == SDLK_i) {
                    int mx = 0;
